...
function mustExists<T>(value: T | null | undefined): T {
...
async function main() {
  ...
  await db
    .insert(userTable)
    .values({ id: userId, email: `e2e-${userId}@test.local` });
  ...
  try {
    ...
    assert(
      mustExists(mum).reminders.length === 1 && mustExists(mum).dueAt !== null,
      'Turn1: reminder window stored',
    );
    ...
    const mumAfter = await getTaskWithReminders(userId, mustExists(mum).id);
    const pending = await listTasksForUser(userId, { status: 'pending' });
    assert(
      mumAfter?.dueAt?.getTime() === friday.getTime(),
      'Turn2: due moved to Friday on the SAME task',
    );
    ...
    const mumDone = await getTaskWithReminders(userId, mustExists(mum).id);
    assert(
      mumDone?.status === 'completed',
      'Turn3: completed cross-chat from chat B',
    );
    ...
    await turn({
      ...
      recentContext:
        'assistant: Want me to add a weekly grocery run?\nuser: yeah add that',
      actions: [
        {
          action: 'create_task',
    ...
    const grocery = (
      await listTasksForUser(userId, { status: 'pending' })
    ).find((t) => t.title === 'Weekly grocery run');
    assert(Boolean(grocery), 'Turn4: accepted suggestion created a Task');
    assert(
      mustExists(grocery).source === 'sophie_accepted',
      'Turn4: accepted suggestion is sourced sophie_accepted',
    );
    ...
    assert(
      refused.committed.length === 0,
      'Turn5: implicit passport refused by the fast path',
    );
    const passportTasks = (await listTasksForUser(userId)).filter((t) =>
      t.title.toLowerCase().includes('passport'),
    );
    assert(
      passportTasks.length === 0,
      'Turn5: no canonical Task from an implicit remark',
    );
    // The same turn still flows to the slow Cortex pass (outbox enqueue).
    const enqueued = await enqueueCortexTurn({
      userId,
      chatId: chatA,
      honchoMessageId: `honcho-${m5}`,
      appMessageId: m5,
      text: 'shit, I need to renew my passport someday',
    });
    outboxIds.push(`honcho-${m5}`);
    assert(
      enqueued.queued === true,
      'Turn5: turn still flows to the slow Cortex pass',
    );
      ...
      actions: [
        {
          ...
          target_task_id: mustExists(grocery).id,
          title: null,
          notes: null,
          due_iso: null,
          reminder_windows: [],
    ...
    const groceryAfter = await getTaskWithReminders(userId, mustExists(grocery).id);
    assert(
      groceryAfter?.status === 'cancelled',
      'Turn6: pronominal cancel resolved to the right task',
    );

    // ── Turn 7: deleting the birth chat leaves the Task intact (chatless) ──
    const gardener = await createTask({
      userId,
      chatId: chatB,
      title: 'Pay the gardener',
    ...
    assert(
      mustExists(gardenerAfter).chatId === null,
      'Turn7: origin chat provenance set to null (SET NULL)',
    );
    assert(
    ...
    const scanRows: Array<Record<string, any>> =
      await serverInitiativeScanCandidates(100, new Date());
    const reminderCandidates = scanRows.filter(
      (c) =>
        String(c.userId) === userId &&
        c.trigger === 'task_reminder' &&
        String(c.context?.taskId) === chatlessReminder.id,
    );
    assert(
      reminderCandidates.length === 1,
      'Turn8: chatless reminder produced an initiative candidate',
    );
    assert(
      typeof reminderCandidates[0]?.chatId === 'string' &&
        String(reminderCandidates[0].chatId) === chatA,
    ...
    assert(
      Boolean(anchored) && anchored.userId === userId,
      'Turn8: delivery chat belongs to the user',
    );
    ...
    const retryActions: InterpreterAction[] = [
      {
        ...
        due_iso: dueMum.toISOString(),
        reminder_windows: [
          { label: 'morning', start_iso: dueMum.toISOString(), end_iso: null },
        ],
        snooze_minutes: null,
        target_resolution: 'referential',
        requires_clarification: false,
    ...
    const calls = (await listTasksForUser(userId)).filter(
      (t) => t.title === 'Call Mum',
    );
    assert(
      calls.length === 1,
      'Turn9: retried create produced exactly one canonical Task',
    );
    assert(
      (await countTurnActions(m1)) === 1,
      'Turn9: retried create recorded exactly one ledger row',
    );
  } finally {
    for (const honchoId of outboxIds) {
      await db
        .delete(cortexOutboxTable)
        .where(eq(cortexOutboxTable.honchoMessageId, honchoId))
        .catch(() => undefined);
    }
    await db
      .delete(messageTable)
      .where(eq(messageTable.chatId, chatA))
      .catch(() => undefined);
    await db
      .delete(chatTable)
      .where(eq(chatTable.id, chatA))
      .catch(() => undefined);
    await db
      .delete(userTable)
