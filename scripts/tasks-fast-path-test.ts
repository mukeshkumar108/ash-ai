...
async function main() {
  // 1. "remind me tomorrow to call Mum" -> exactly one canonical Task/reminder
  {
    ...
    await db
      .insert(userTable)
      .values({ id: userId, email: `fp-${userId}@test.local` });
    await db
      .insert(chatTable)
      .values({ id: chatId, userId, title: 'fp chat', createdAt: new Date() });
    ...
    try {
      ...
      assert(
        pending.length === 1 && pending[0].title === 'Call Mum',
        'exactly one canonical Task/reminder',
      );
      assert(
        pending[0].source === 'conversation',
        'create source is conversation',
      );
      assert(
        pending[0].sourceMessageId === messageId,
        'app message provenance stored',
      );
      assert(pending[0].reminders.length === 1, 'reminder window persisted');
      assert(
        (await countTurnActions(messageId, 'created')) === 1,
        'one created ledger row',
      );
    } finally {
  ...
  // 2. "actually make that Friday" -> same Task rescheduled, not a duplicate
  {
    ...
    try {
      ...
      assert(
        result.committed.length === 1 && result.committed[0].taskId === mum.id,
        'reschedule committed on the same task',
      );
      assert(all.length === 2, 'still two tasks (no duplicate created)');
      assert(
        refreshed?.dueAt?.getTime() === friday.getTime(),
        'due moved to Friday on the SAME task',
      );
      assert(
        (await countTurnActions(messageId, 'updated')) === 1,
        'one updated ledger row',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
  ...
  // 3. "I did it" unambiguous (single roster) -> correct task completed
  {
    ...
    try {
      ...
      assert(
        ...
        'complete committed (roster was unique)',
      );
      assert(
        refreshed?.status === 'completed',
        'correct task completed when unambiguous',
      );
      assert(
        (await countTurnActions(messageId, 'completed')) === 1,
  ...
  // 4. "I did it" with two equally plausible tasks -> no mutation + clarification
  {
    ...
    try {
      ...
      assert(
        ...
        'no mutation on ambiguous reference',
      );
      assert(
        result.clarifications.some((c) => c.intent === 'ambiguous_target'),
        'narrow clarification surfaced',
      );
      assert(
        all.length === 2 && all.every((t) => t.status === 'pending'),
        'no task was completed',
      );
      assert(
        (await countTurnActions(messageId, 'completed')) === 0,
        'no ledger row for the rejected ambiguity',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
  ...
  // 5. "cancel that" resolved through recent context
  {
    ...
    try {
      ...
      assert(
        result.committed.length === 1 &&
          result.committed[0].taskId === dentist.id,
        'cancel committed on context-grounded target',
      );
      assert(refreshed?.status === 'cancelled', 'correct task cancelled');
      assert(
        (await countTurnActions(messageId, 'cancelled')) === 1,
        'one cancelled ledger row',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
  ...
  // 6. "remind me tomorrow to call Mum and Friday to send the form" -> two actions, no dupes
  {
    ...
    await db.insert(messageTable).values({
      ...
      parts: [
        {
          type: 'text',
          text: 'remind me tomorrow to call Mum and Friday to send the form',
        },
      ],
      attachments: [],
      createdAt: new Date(),
    ...
    try {
      ...
      assert(
        all.some((t) => t.title === 'Call Mum') &&
          all.some((t) => t.title === 'Send the form'),
        'both titles present',
      );
      assert(
        (await countTurnActions(messageId, 'created')) === 2,
        'two created ledger rows',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
  ...
  // 7. cross-chat completion still works
  {
    ...
    try {
      ...
      assert(
        refreshed?.status === 'completed',
        'task born in another chat completed from this chat',
      );
    } finally {
  ...
  // 8. chatless/manual task remains working and completable via fast path
  {
    ...
    try {
      ...
      assert(
        manual.chatId === null,
        'manual task survives chatless (provenance null)',
      );
      const result = await commitTurnSemantics({
        userId,
        chatId,
      ...
      assert(
        result.committed.length === 1,
        'chatless manual task completed via fast path',
      );
      assert(
        refreshed?.status === 'completed',
        'chatless/manual task behaviour intact',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
  ...
  // 9. retry of the same action does not duplicate Task/TurnAction
  {
    ...
    try {
      ...
      const first = await commitTurnSemantics({
        ...base,
        generate: interpret(),
      });
      const second = await commitTurnSemantics({
        ...base,
        generate: interpret(),
      });
      const all = await listTasksForUser(userId, { status: 'pending' });
      ...
      assert(
        all.length === 1 && all[0].title === 'Renew passport',
        'exactly one canonical Task after retry',
      );
      assert(
        (await countTurnActions(messageId, 'created')) === 1,
        'exactly one created ledger row after retry',
      );
    } finally {
      await cleanup(userId, [chatId]);
    }
